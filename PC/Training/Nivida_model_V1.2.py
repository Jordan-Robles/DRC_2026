...
datadir = r'/Users/adamsavage/StudioProjects/DRC_2025/Testing_Data/Test4&5/' # remmebr to chabnge this directory to the new direcotry for the data sets
...
samples_per_bin = 700 # 400, ensures a max value per bin of 200 to ensure the model isnt bias towards driving straight
...
image_paths, steerings = load_img_steering(r'/Users/adamsavage/StudioProjects/DRC_2025/Testing_Data/Test4&5/', data) # two arrays, 1 for images and other for array conataining each images corresponding steering angle
...
def img_preprocess(img): #pre-process our data to be used inside our model
    ...
    img = cv2.cvtColor(img, cv2.COLOR_BGR2LAB) #converts image to YUV as perscribed by nivida, Y = lumninsoity, UV = chronmium which adds color to the image
...
image = image_paths[50] #selects the image x to he used
...
def nvidia_model(): #uses 200x66 images
    ...
    optimizer = Adam(learning_rate = 1e-4) #lower learning rate to reduce jagged (noise) training and validation loss curve
